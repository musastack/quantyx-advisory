...
/* Cash flow — weekly rolling 8 weeks */
const cashFlowData = [
...
function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`p-6 rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#0d0d1a] shadow-sm dark:shadow-none ${className}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <Card className={`!p-5 ${glow}`}>
      <div className="flex items-center justify-between mb-3">
        <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-500 dark:text-white/35">{label}</p>
        <div className={`w-8 h-8 rounded-lg ${bg} flex items-center justify-center`}>
          <Icon size={15} className={color} />
        </div>
      </div>
      <p className="text-2xl font-bold tracking-tight mb-1 text-slate-900 dark:text-white font-mono">{value}</p>
...
function Signal({ type, title, detail, action }: { type: "alert" | "warning" | "info"; title: string; detail: string; action?: string }) {
  const styles = {
    alert:   { border: "border-l-rose-500",   bg: "bg-rose-50   dark:bg-rose-500/[0.06]   border border-rose-200   dark:border-rose-500/15",   text: "text-rose-700   dark:text-rose-300",   icon: <AlertCircle size={13} className="text-rose-500 shrink-0 mt-0.5" /> },
    warning: { border: "border-l-amber-500",  bg: "bg-amber-50  dark:bg-amber-500/[0.06]  border border-amber-200  dark:border-amber-500/15",  text: "text-amber-700  dark:text-amber-300",  icon: <AlertTriangle size={13} className="text-amber-500 shrink-0 mt-0.5" /> },
    info:    { border: "border-l-indigo-500", bg: "bg-indigo-50 dark:bg-indigo-500/[0.06] border border-indigo-200 dark:border-indigo-500/15", text: "text-indigo-700 dark:text-indigo-300", icon: <Activity size={13} className="text-indigo-500 shrink-0 mt-0.5" /> },
  };
  const s = styles[type];
  return (
  ...
  return (
    <div className="flex items-start justify-between mb-5">
      <div>
        ...
        <p className="text-xs text-slate-400 dark:text-white/35 mt-0.5">{sub}</p>
      </div>
      {badge && (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <KpiCard label="Overdue receivables" value={`£${(overdueTotal / 1000).toFixed(0)}k`}          change={`${overdue.length} accounts`}         up={false} icon={AlertCircle}  color="text-rose-600 dark:text-rose-400"     bg="bg-rose-100 dark:bg-rose-500/10"     glow="glow-rose"    sub="oldest: 52 days" />
        <KpiCard label="Gross margin"        value={`${avgMargin}%`}                                   change="–0.8pp vs last month"      up={false} icon={TrendingDown}  color="text-violet-600 dark:text-violet-400" bg="bg-violet-100 dark:bg-violet-500/10" glow=""             sub="Apr 2026" />
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <Card className="lg:col-span-1">
          <SectionHeader title="What needs attention" sub="Flagged this week" badge="Live" />
          <div className="space-y-2.5">
            <Signal type="alert"   title={`£${(overdueTotal/1000).toFixed(0)}k overdue — ${overdue.length} invoices`}  detail="INV-0339 (Prestige Retail) is 52 days overdue. Auto-escalation triggered." action="Review receivables" />
            <Signal type="alert"   title="Crew D overallocated at 98%"                                                  detail="8 active jobs, 196 hrs logged. Reallocation required before week end."       action="View crew plan" />
            <Signal type="warning" title="Job #2851 labour 11% over estimate"                                           detail="Civic Centre Renovation. £53.8k actual vs £48.2k budgeted at 43% complete." action="Review job" />
            <Signal type="warning" title="Gross margin declining — 35.1% in Apr"                                        detail="Down from 38.9% in Oct. Three low-margin jobs dragging the blended rate."   action="View margin detail" />
            <Signal type="info"    title="£47.1k WIP not yet invoiced — Avonside"                                       detail="Job #2847 complete. Draft invoice ready. Raising now would improve cash position." action="Raise invoice" />
          </div>
        </Card>

        {/* Cash trend */}
        <Card className="lg:col-span-2">
          <SectionHeader title="Cash balance — rolling 8 weeks" sub="Weekly in/out · bank account view" />
          ...
          <div className="mt-3 p-3 rounded-xl border border-amber-200 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/[0.04] flex items-start gap-2">
            <AlertTriangle size={13} className="text-amber-500 shrink-0 mt-0.5" />
            <p className="text-[11px] text-amber-700 dark:text-amber-300/80">Balance dropped to £116k w/e 14 Apr — lowest in 8 weeks. Collecting INV-0339 and INV-0341 this week would add £49.6k.</p>
          </div>
        </Card>
      </div>
    </div>
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          ...
          { label: "Cash balance",  value: "£161.6k",                             sub: "as of 21 Apr",                color: "text-indigo-700 dark:text-indigo-300", border: "border-indigo-200 dark:border-indigo-500/20", bg: "bg-indigo-50  dark:bg-white/[0.025]" },
        ].map(s => (
      ...
      <Card>
        ...
        <div className="space-y-2.5">
          <Signal type="alert"   title="INV-0339 — Prestige Retail, 52 days overdue (£31,200)" detail="Exceeds 45-day escalation threshold. Auto-chaser sent. Recommend direct contact." />
          <Signal type="alert"   title="INV-0341 — Hartley Council, 38 days overdue (£18,400)" detail="Public sector client — payment terms 30 days. Query raised, awaiting PO confirmation." />
          <Signal type="warning" title="Cash balance at 8-week low — £116.3k (w/e 14 Apr)"     detail="Outflows exceeded inflows by £49.6k. Recovery expected if overdue invoices collected." />
          <Signal type="info"    title="£165.2k WIP eligible for invoicing across 4 jobs"       detail="Raising these now would materially improve the cash position before month end." />
        </div>
      </Card>
    </div>
  );
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Apr gross margin", value: `${latestMargin}%`, sub: `${marginDelta}pp since Oct`, color: "text-rose-600 dark:text-rose-400", border: "border-rose-200 dark:border-rose-500/20", bg: "bg-rose-50 dark:bg-white/[0.025]" },
        ].map(s => (
          <div key={s.label} className={`p-5 rounded-2xl border ${s.border} ${s.bg} shadow-sm dark:shadow-none`}>
            <p className="text-xs text-slate-500 dark:text-white/40 mb-2">{s.label}</p>
      ...
      {/* Margin trend */}
      <Card>
        <SectionHeader title="Gross margin trend" sub="Oct 2025 – Apr 2026" />
        ...
        <p className="text-[11px] text-rose-600 dark:text-rose-400/80 mt-3">⚠ Margin has compressed –3.8pp since Oct. Three low-margin jobs are the primary driver.</p>
      </Card>

      {/* Margin leaks */}
      <Card>
        <SectionHeader title="Margin leaks — jobs below 30% gross margin" sub={`${lowMarginJobs.length} jobs flagged`} badge="Exception" />
            ...
            return (
              <div key={job.id} className="p-4 rounded-xl border border-slate-200 dark:border-white/[0.07] bg-slate-50 dark:bg-white/[0.02]">
                <div className="flex items-center justify-between gap-4 mb-2">
                  ...
                  <div className="text-right shrink-0">
                    <p className={`text-lg font-bold font-mono ${job.margin < 25 ? "text-rose-600 dark:text-rose-400" : "text-amber-600 dark:text-amber-400"}`}>{job.margin}%</p>
      ...
      {/* Margin signals */}
      <Card>
        <SectionHeader title="Margin intelligence" sub="Automated signals" badge="System" />
        <div className="space-y-2.5">
          <Signal type="alert"   title="Job #2867 — Factory Mezzanine at 19.2% margin"      detail="Steel bracket shortage adding cost. Currently 88% complete. Risk of further slippage before completion." />
          <Signal type="alert"   title="Job #2859 — Prestige Retail at 22.1% margin"         detail="Site access delays adding unbudgeted prelim costs. Client yet to agree variation order." />
          <Signal type="warning" title="Blended margin down –3.8pp since Oct 2025"           detail="Volume growth is masking margin erosion. Three jobs below 30% are compressing the overall rate." />
          <Signal type="info"    title="Metro Property Group jobs averaging 40.6% margin"    detail="Highest-margin client. Prioritising capacity allocation to this client would improve the blended rate." />
        </div>
      </Card>
    </div>
  );
...
function OperationsSection() {
  ...
  const labourData = crews.map(c => ({ crew: c.name, hours: c.hours }));
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Clients with overdue debt",  value: `${highRisk.length}`,  sub: "requiring attention",  color: "text-rose-600 dark:text-rose-400",   border: "border-rose-200 dark:border-rose-500/20",   bg: "bg-rose-50 dark:bg-white/[0.025]"  },
          { label: "Avg gross margin by client", value: "34.8%", sub: "blended across all clients", color: "text-slate-900 dark:text-white", border: "border-slate-200 dark:border-white/[0.07]", bg: "bg-white dark:bg-white/[0.025]" },
        ].map(s => (
          <div key={s.label} className={`p-5 rounded-2xl border ${s.border} ${s.bg} shadow-sm dark:shadow-none`}>
            <p className="text-xs text-slate-500 dark:text-white/40 mb-2">{s.label}</p>
            <p className={`text-2xl font-bold font-mono ${s.color}`}>{s.value}</p>
              ...
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-500 dark:text-white/35">{c.jobs} active jobs · {c.pct}% of revenue</p>
                  </div>
                </div>
      ...
      <Card>
        ...
        <div className="space-y-2.5">
          <Signal type="alert"   title="Prestige Retail — £31,200 overdue, 52 days"              detail="Lowest-margin client (30.1%) and highest overdue exposure. Site delay dispute may affect collectability." action="Chase now" />
          ...
          <Signal type="warning" title="Barton Industrial — margin at 28.1%, below target"       detail="Second-lowest margin client. Factory Mezzanine has materials shortage — further margin at risk." />
          <Signal type="warning" title="Avonside Commercial — 37.1% revenue concentration"       detail="Single client accounts for over a third of YTD revenue. Dependency risk if relationship deteriorates." />
          <Signal type="info"    title="Metro Property Group — highest margin at 40.0%"           detail="Consistent performer, no overdue debt. Prioritise capacity allocation to grow this relationship." />
        </div>
      </Card>
    </div>
  );
...
function AlertsSection() {
  const allSignals = [
    { type: "alert"   as const, area: "Cash",       title: "INV-0339 — Prestige Retail, 52 days overdue",           detail: "£31,200. Exceeds 45-day escalation threshold. Auto-chaser sent." },
    ...
    { type: "warning" as const, area: "Operations", title: "Job #2851 — labour 11% over estimate at 43% complete",  detail: "Civic Centre. Projected overrun £8–12k." },
    { type: "warning" as const, area: "Materials",  title: "Steel bracket shortage — 5-day lead time confirmed",    detail: "Affects #2867. Alternative supplier not yet sourced." },
    ...
    { type: "info"    as const, area: "Cash",       title: "£165.2k WIP eligible for invoicing",                    detail: "Raising invoices now would materially improve month-end cash position." },
    { type: "info"    as const, area: "Customers",  title: "Metro Property Group — 40.0% margin, no overdue debt",  detail: "Highest-value client relationship. Capacity prioritisation recommended." },
  ];
...
const SECTION_HEADERS: Record<string, { title: string; sub: string }> = {
  "Overview":           { title: "Executive overview — April 2026",  sub: "Meridian Contractors Ltd · Operating system"           },
  ...
  "Operations":         { title: "Operations",                       sub: "Jobs, crews, labour utilisation, and materials"        },
...
export default function Dashboard() {
  ...
  const alertCount = 4; // critical alerts

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#05050f] text-slate-900 dark:text-white">

      {/* ── Sidebar ── */}
      <aside className="fixed left-0 top-0 h-full w-60 border-r border-slate-200 dark:border-white/[0.06] bg-white dark:bg-[#08080f] flex flex-col z-40 hidden lg:flex shadow-sm dark:shadow-none">
        <div className="p-5 border-b border-slate-100 dark:border-white/5">
          <p className="text-[9px] font-semibold uppercase tracking-widest text-slate-400 dark:text-white/25 mb-1">Custom operating system</p>
          <span className="font-bold text-sm text-slate-900 dark:text-white">Meridian Contractors</span>
          <div className="mt-2 flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
            <p className="text-xs text-slate-400 dark:text-white/35">Live · synced 4 min ago</p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-white/5 space-y-3">
          <div className="px-3 py-2.5 rounded-lg bg-indigo-50 dark:bg-indigo-500/[0.07] border border-indigo-200 dark:border-indigo-500/20">
            <p className="text-[9px] font-semibold text-indigo-600 dark:text-indigo-400/80 uppercase tracking-widest mb-0.5">Built by</p>
      ...
      <div className="lg:ml-60">
        ...
        {/* Top bar */}
        <header className="sticky top-0 z-30 h-14 border-b border-slate-200 dark:border-white/[0.06] bg-white/90 dark:bg-[#05050f]/90 backdrop-blur-md flex items-center justify-between px-6">
          <div>
